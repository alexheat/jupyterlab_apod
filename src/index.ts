...
const plugin: JupyterFrontEndPlugin<void> = {
  ...
  activate: async (app: JupyterFrontEnd, palette: ICommandPalette) => {
    console.log(`JupyterLab extension jupyterlab_apod is activated! ${Date()}`);
    ...
    const newWidget = async () => {
      ...
      // Add a div element to the content
      ...
      // Save the random date to a variable
      const randomDateString: string = randomDate();

      div.innerText = randomDateString

      // Fetch info about a random picture
      const response = await fetch(`https://api.nasa.gov/planetary/apod?api_key=DEMO_KEY&date=${randomDateString}`);
